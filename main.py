...
def getheaders(token=None, content_type="application/json"):
    headers = {
        ...
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.11 (KHTML, like Gecko) Chrome/23.0.1271.64 Safari/537.11" # ne pas toucher sinon detect
    }
    if token:
...
def bot_title():
    os.system("cls")
    ctypes.windll.kernel32.SetConsoleTitleW(f"Fake Verif Bot")
#Bot home page
def startprint():
    bot_title()
    ...
    print(f"""                                            {y}[{b}+{y}]{w} Bot Informations:\n
                                                [#] Nom du bot:    {bot.user.name}
                                                [#] ID du bot:          {bot.user.id}
                                                [#] Salon de Logs:    {logs_channel_id if logs_channel_id != "LOGS-CHANNEL-ID-HERE" else "None"}
                                                [#] Nom de la Commande:    {bot.command_prefix}{command_name}\n\n
                                            {y}[{b}+{y}]{w} Settings View:\n
                                                [#] Role:       {give_role_texte}
...
bot = commands.Bot(command_prefix=prefix, description="Fake Verification Bot - Made by Elise", intents=intents)
...
def Init():
    ...
    if botToken == "":
        bot_title()
        input(f"{y}[{Fore.LIGHTRED_EX}!{y}]{w} Veuillez insérer un token dans le fichier config.json.")
        return
    elif prefix == "":
        bot_title()
        input(f"{y}[{Fore.LIGHTRED_EX}!{y}]{w} Veuillez insérer un prefix dans le fichier config.json file.")
        return
    try:
        bot.run(botToken)
    except:
        os.system("cls")
        bot_title()
        input(f"{y}[{Fore.LIGHTRED_EX}!{y}]{w} Le token présent dans le fichier config.json est invalide.")
        return
                    ...
                    if give_role == True:
                        try:
                            ...
                            if logs_channel:
                                ...
                                embed_role.set_footer(text="Made by Elise  ・  https://Elise")
                    ...
                    if mass_dm == 1 or mass_dm == 3:
                        try:
                            ...
                            if logs_channel:
                                ...
                                embed_cdm.set_footer(text="Made by Elise  ・  https://Elise")
                    ...
                    if mass_dm == 2 or mass_dm == 3:
                        try:
                            ...
                            if logs_channel:
                                ...
                                embed_fdm.set_footer(text="Made by Elise  ・  https://Elise")
